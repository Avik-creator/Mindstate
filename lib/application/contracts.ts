...
export const scopeSchema = z.enum(['memory:read', 'memory:write'])
export const scopesSchema = z.array(scopeSchema).min(1).max(2).transform((scopes) => [...new Set(scopes)])
...
export const signupTokenCreateSchema = z.object({
  ...
  scopes: scopesSchema.default(['memory:read', 'memory:write']),
