...
export async function GET(request: Request) {
  ...
  if (!can(actor, 'handoff:read')) return NextResponse.json({ error: { code: 'FORBIDDEN', message: 'handoff:read scope required' } }, { status: 403 })
  return NextResponse.json({ data: await workspaceService.listHandoffs(actor) })
}
export async function POST(request: Request) {
  const actor = await actorFromRequest(request); if (!actor) return NextResponse.json({ error: { code: 'UNAUTHORIZED', message: 'Authentication required' } }, { status: 401 })
  if (!can(actor, 'handoff:write')) return NextResponse.json({ error: { code: 'FORBIDDEN', message: 'handoff:write scope required' } }, { status: 403 })
