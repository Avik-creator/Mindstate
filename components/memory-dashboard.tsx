...
import { Activity, ArrowUpRight, Bot, Braces, Check, CircleDot, Code2, Database, FileText, Folder, Layers3, LogOut, Menu, Plus, Search, Terminal } from 'lucide-react'
...
function SharedSourceMap({ agents, memories, handoffs }: { agents: Agent[]; memories: Memory[]; handoffs: Handoff[] }) {
  const visibleAgents = agents.slice(0, 3)
  const fallbackAgents = [
    { id: 'opencode', name: 'OpenCode', runtimeName: 'Ready to connect', status: 'available', category: 'coding', capabilities: [], confidence: 'unverified', lastSeenAt: null },
    { id: 'codex', name: 'Codex', runtimeName: 'Ready to connect', status: 'available', category: 'coding', capabilities: [], confidence: 'unverified', lastSeenAt: null },
  ]
  const nodes = visibleAgents.length ? visibleAgents : fallbackAgents
  const icons = [Terminal, Code2, Bot]

  return <Card className="overflow-hidden"><CardHeader className="border-b"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><CardTitle>One source, every agent</CardTitle><CardDescription className="mt-1 max-w-xl">Each tool has its own identity. They meet in the same private memory, so work can move without context loss.</CardDescription></div><Badge variant="secondary" className="w-fit font-mono text-[9px] uppercase">{agents.length || 0} connected</Badge></div></CardHeader><CardContent className="p-6"><div className="flex flex-col items-center gap-4"><div className="grid w-full gap-3 sm:grid-cols-3">{nodes.map((agent, index) => { const Icon = icons[index] ?? Bot; return <div key={agent.id} className="rounded-xl border bg-background p-4"><div className="flex items-center justify-between gap-3"><span className="flex size-9 items-center justify-center rounded-lg bg-secondary"><Icon className="size-4" /></span><span className={`size-2 rounded-full ${agent.lastSeenAt ? 'bg-primary' : 'bg-muted-foreground/40'}`} /></div><h3 className="mt-3 text-sm font-medium">{agent.name}</h3><p className="mt-1 text-xs text-muted-foreground">{agent.runtimeName ?? agent.category}</p></div>})}</div><div className="flex h-7 items-center justify-center" aria-hidden="true"><div className="h-7 w-px bg-border" /><div className="absolute h-px w-1/2 bg-border" /></div><div className="flex w-full max-w-md items-center gap-4 rounded-xl border border-primary/30 bg-secondary/50 p-4"><span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-primary text-primary-foreground"><Database className="size-5" /></span><div><h3 className="text-sm font-semibold">Your shared mindstate</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">{memories.length} memories and {handoffs.filter(h => h.status === 'open').length} open handoffs available to every enrolled agent.</p></div></div></div></CardContent></Card>
}
